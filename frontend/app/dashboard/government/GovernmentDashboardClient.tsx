...
function statusBadge(status: string) {
  ...
  return m[status] ?? 'bg-slate-500/20 text-slate-400 border border-slate-500/30'
}
...
function slaBadge(level: string) {
  const m: Record<string, string> = {
    ...
    no_sla:   'bg-slate-500/20 text-slate-500 border border-slate-700',
  }
  return m[level] ?? 'bg-slate-500/20 text-slate-500 border border-slate-700'
}
...
function capacityBadge(status: string) {
  ...
  return m[status] ?? 'bg-slate-500/15 text-slate-400 border-slate-500/30'
}
        ...
        <React.Fragment key={s}>
          <div className="flex flex-col items-center gap-1 min-w-0 shrink-0">
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border-2 transition-all ${
              s === status
                ? 'bg-indigo-500 border-indigo-400 text-white shadow-sm shadow-indigo-500/40 scale-110'
                : i < idx
            ...
            <span className={`text-[9px] leading-tight text-center w-12 font-medium ${
              s === status ? 'text-indigo-400' : i < idx ? 'text-emerald-600' : 'text-slate-700'
            }`}>{PIPELINE_LABELS[i]}</span>
          </div>
          {i < PIPELINE.length - 1 && (
            <div className={`flex-1 h-0.5 mx-0.5 min-w-3 ${i < idx ? 'bg-emerald-500/40' : 'bg-slate-800'}`} />
          )}
        </React.Fragment>
      ))}
    </div>
...
function KpiCard({ label, value, sub, color, icon }: {
  ...
  return (
    <div className="glass-card p-5 flex items-start gap-4 hover:-translate-y-1 transition-transform">
      <div className={`shrink-0 w-11 h-11 rounded-xl ${color} flex items-center justify-center`}>{icon}</div>
      <div>
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{label}</p>
        <p className="text-2xl font-extrabold text-white leading-none mt-0.5">{value}</p>
...
function Tab({ label, active, onClick, badge }: {
  ...
  return (
    ...
    >
      {label}
      {badge != null && badge > 0 && (
        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${active ? 'bg-white/20 text-white' : 'bg-red-500/20 text-red-400'}`}>
          {badge}
        </span>
      )}
    </button>
...
export default function GovernmentDashboardClient({
  ...
  const ManualAssignModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div className="glass-card w-full max-w-2xl max-h-[85vh] flex flex-col border border-white/20 shadow-2xl">
          <div className="p-5 border-b border-white/10 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Manual Routing Override</h2>
              <p className="text-xs text-slate-400">Assign Ticket #{assignModalTicket.id} to a specific institution.</p>
            </div>
          ...
          <div className="p-5 overflow-y-auto space-y-3">
            ...
            {workload.sort((a,b) => b.reputation_score - a.reputation_score).map(inst => {
              ...
              return (
                <div key={inst.id} className="flex items-center justify-between p-4 rounded-xl border border-white/10 hover:border-indigo-500/50 bg-slate-900/50 transition-colors">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isDomainMatch && <span className="text-[9px] px-1.5 py-0.5 rounded uppercase font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">Domain Match</span>}
                    </div>
                    <div className="text-xs text-slate-500 flex gap-4">
                      <span>Type: <span className="text-slate-300 capitalize">{inst.type}</span></span>
                      <span>Reputation: <span className="text-indigo-400 font-bold">{inst.reputation_score.toFixed(2)}</span></span>
                      <span>Load: <span className={`font-bold ${isOverloaded ? 'text-red-400' : 'text-emerald-400'}`}>{inst.current_load}</span> active</span>
                    </div>
                  </div>
                  <form action={async () => {
                    ...
                    <button type="submit" className="text-xs font-semibold px-4 py-2 rounded-lg bg-indigo-600/80 hover:bg-indigo-600 text-white transition-colors border border-indigo-500/40 shadow-sm">
                      Assign Here
                    </button>
                  </form>
                </div>
  ...
  const OverviewTab = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <KpiCard label="Total Tickets" value={stats.total_tickets} sub="Platform wide" color="bg-indigo-500/20"
        ...
        <KpiCard label="Resolved" value={stats.closed_tickets} sub={`${stats.resolution_rate}% rate`} color="bg-emerald-500/20"
          icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
        ...
        <div className="rounded-2xl bg-red-500/8 border border-red-500/20 p-4 flex flex-wrap items-start gap-3 animate-pulse">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1">
              {slaAlerts > 0 && <button onClick={() => setActiveTab('sla')} className="text-xs text-slate-300 hover:text-white underline underline-offset-2">⏰ {slaAlerts} SLA breach/critical</button>}
              {staleCount > 0 && <button onClick={() => setActiveTab('stale')} className="text-xs text-slate-300 hover:text-white underline underline-offset-2">😴 {staleCount} stale ticket{staleCount !== 1 ? 's' : ''} (no activity 24h)</button>}
              {stats.escalated_tickets > 0 && <button onClick={() => { setStatusFilter('escalated'); setActiveTab('tickets') }} className="text-xs text-slate-300 hover:text-white underline underline-offset-2">⚡ {stats.escalated_tickets} escalated</button>}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="glass-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Issue Pipeline Tracker</h2>
          <p className="text-xs text-slate-600">Top domain: <span className="text-slate-300 font-semibold">{stats.top_domain}</span> ({stats.top_domain_count} tickets)</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {Object.entries(stats.status_breakdown).sort((a,b) => b[1]-a[1]).map(([status, count]) => {
            ...
            const colors: Record<string, string> = {
              ...
              in_progress:'bg-cyan-500', piloting:'bg-teal-500', verified:'bg-emerald-400',
              closed:'bg-slate-500', escalated:'bg-red-500',
            }
            return (
              <button key={status} onClick={() => { setStatusFilter(status); setActiveTab('tickets') }}
                className="flex items-center gap-1.5 text-xs text-slate-400 bg-slate-900/60 border border-white/5 px-2.5 py-1.5 rounded-lg hover:border-white/20 hover:text-white transition-all">
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-xs text-slate-500 mb-1.5">
            ...
            <span className="font-bold text-white">{stats.resolution_rate}%</span>
          </div>
          <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-teal-500" style={{ width: `${Math.min(stats.resolution_rate, 100)}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-5 hover:border-indigo-500/20 transition-colors">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Institution Capacity Watch</h2>
            <button onClick={() => setActiveTab('routing')} className="text-xs text-indigo-400 hover:underline">View Intelligence →</button>
          </div>
          {workload.length === 0 ? (
              ...
              {workload.slice(0, 4).map(inst => (
                <div key={inst.id} className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${
                          ...
                          inst.capacity_status === 'moderate' ? 'bg-amber-500' : 'bg-emerald-500'
                  ...
                  <div className="shrink-0 text-right">
                    <p className="text-sm font-bold text-indigo-400">{inst.reputation_score.toFixed(2)}</p>
        ...
        <div className="glass-card p-5 hover:border-indigo-500/20 transition-colors">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">SLA Health Overview</h2>
            <button onClick={() => setActiveTab('sla')} className="text-xs text-indigo-400 hover:underline">Monitor SLAs →</button>
          </div>
          {!slaRisk ? (
            ...
            <div className="space-y-2">
              {[
                ...
                { label: '🟢 On Track', count: slaRisk.summary.safe_count, color: 'bg-emerald-500' },
              ].map(item => (
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-bold text-slate-300 w-5 text-right">{item.count}</span>
                </div>
              ))}
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="relative group">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none group-focus-within:text-indigo-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          <input type="text" placeholder="Search by title, description, domain, ID, institution..." value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full pl-10 pr-4 py-3 bg-slate-900 border border-white/10 rounded-xl text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 transition-all shadow-inner shadow-black/20" />
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(['all', ...Object.keys(statusCounts).filter(k => k !== 'all')] as string[]).map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`text-xs px-3 py-1.5 rounded-xl border font-semibold transition-all capitalize ${statusFilter === s ? 'bg-indigo-600 border-indigo-500 text-white shadow-sm shadow-indigo-500/30' : 'bg-white/5 border-white/10 text-slate-400 hover:text-white hover:bg-white/10'}`}>
          ...
          {domains.length > 0 && <>
            ...
            <select value={domainFilter} onChange={e => setDomainFilter(e.target.value)}
              className="text-xs px-3 py-1.5 bg-slate-900 border border-white/10 rounded-xl text-slate-300 focus:outline-none focus:border-indigo-500/50 cursor-pointer hover:border-white/20 transition-colors">
              <option value="all">All Domains</option>
              {domains.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
          </>}
          <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)}
            className="text-xs px-3 py-1.5 bg-slate-900 border border-white/10 rounded-xl text-slate-300 focus:outline-none focus:border-indigo-500/50 cursor-pointer ml-auto hover:border-white/20 transition-colors">
      ...
      <p className="text-xs text-slate-600">Showing <span className="text-slate-300 font-semibold">{filtered.length}</span> of <span className="text-slate-300 font-semibold">{tickets.length}</span> tickets</p>

      {/* Ticket Cards */}
      {filtered.length === 0 ? (
        <div className="glass-card p-12 text-center">
          <p className="text-4xl mb-2">🔍</p>
          <p className="text-slate-300 font-medium text-lg">No tickets match criteria</p>
        ...
        <div className="flex flex-col gap-3">
          {filtered.map(ticket => {
            ...
            return (
              <div key={ticket.id} className={`glass-card transition-all ${
                isEscalated ? 'border-red-500/30 bg-red-500/5' : slaUrgent ? 'border-orange-500/25 bg-orange-500/5' : isCritical ? 'border-amber-500/20' : 'border-white/10 hover:border-indigo-500/30'
              }`}>
                ...
                <div className="p-5 cursor-pointer group" onClick={() => setExpandedId(isExpanded ? null : ticket.id)}>
                  <div className="flex items-start gap-4">
                    ...
                    <div className="shrink-0 flex flex-col items-center gap-1 pt-0.5">
                      <div className="w-1.5 h-16 bg-slate-800 rounded-full overflow-hidden flex flex-col-reverse shadow-inner">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                            ...
                            {ticket.classification_confidence != null && (
                              <span className="text-[9px] text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-1.5 py-0.5 rounded" title="AI Classification Confidence">
                                AI 🤖 {(ticket.classification_confidence * 100).toFixed(0)}%
                              </span>
                            )}
                          </div>
                          <h3 className="font-bold text-white text-[15px] leading-snug group-hover:text-indigo-300 transition-colors">{ticket.title}</h3>
                        </div>
                      ...
                      <p className="text-sm text-slate-400 line-clamp-2 mb-3 mt-1 leading-relaxed">{ticket.description}</p>

                      {/* Meta tags */}
                      <div className="flex flex-wrap items-center gap-2">
                        {ticket.domain && (
                          <span className="text-xs font-mono bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 rounded-md shadow-sm">{ticket.domain}</span>
                        )}
                        ...
                        {ticket.assigned_institution ? (
                          ...
                          <span className="text-xs text-slate-500 bg-slate-800/80 px-2 py-0.5 rounded-md border border-slate-700">Unassigned</span>
                        )}
                        {ticket.sla_hours_remaining != null && (
                          <span className={`text-xs px-2 py-0.5 rounded-md font-mono font-semibold ${ticket.sla_hours_remaining < 0 ? 'text-red-400 bg-red-500/10' : ticket.sla_hours_remaining < 24 ? 'text-amber-400 bg-amber-500/10' : 'text-slate-400 bg-slate-800'}`}>
                        ...
                        {ticket.contact_phone && (
                          <a href={`tel:${ticket.contact_phone}`} onClick={e => e.stopPropagation()}
                            className="text-xs text-indigo-300 hover:text-white flex items-center gap-1 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-md transition-colors shadow-sm">
                            📞 {ticket.contact_phone}
                          </a>
                        )}
                  ...
                  <div className="border-t border-white/5 bg-slate-900/30">
                    <div className="p-5 space-y-5">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 overflow-x-auto pb-2">
                            {ticket.media_urls.map((url, i) => (
                              <a key={i} href={`/${url}`} target="_blank" rel="noreferrer" className="shrink-0 block h-20 w-28 rounded-lg bg-slate-800 border border-white/10 hover:border-indigo-500 transition-colors overflow-hidden relative group">
                                <span className="absolute inset-0 flex items-center justify-center text-xs text-slate-400 font-medium group-hover:text-indigo-300 transition-colors z-10">View Media</span>
                              </a>
                            ))}
                          </div>
                        </div>
                      ...
                      {lastUpdate && ticket.status !== 'closed' && (
                        <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 flex gap-3 shadow-inner">
                          ...
                          <div>
                            <p className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-1">Latest Progress: {lastUpdate.type.replace(/_/g,' ')}</p>
                            <p className="text-sm text-slate-200">{lastUpdate.notes}</p>
                            <p className="text-[10px] text-slate-500 mt-1">{timeAgo(lastUpdate.time)}</p>
                          </div>
                        </div>
                      )}

                      {/* Resolution & Team Attribution */}
                      {(ticket.completion_notes || (ticket.worker_credits && ticket.worker_credits.length > 0) || (ticket.proof_media_urls && ticket.proof_media_urls.length > 0)) && (
                        <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20 space-y-3">
                          <div className="flex items-center justify-between pb-1 border-b border-emerald-500/15">
                            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-1.5">
                              <span>👷</span>
                              <span>Resolution Summary & Team Credits</span>
                            </span>
                            <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">
                              {ticket.status === 'piloting' ? 'Awaiting Verification' : 'Verified Resolution'}
                            </span>
                          </div>
                          {ticket.completion_notes && (
                            <div>
                              <p className="text-[11px] text-slate-400 font-semibold mb-1">Implementation Notes:</p>
                              <p className="text-xs text-slate-200 bg-black/20 p-2.5 rounded-lg border border-white/5 leading-relaxed">{ticket.completion_notes}</p>
                            </div>
                          )}
                          {ticket.worker_credits && ticket.worker_credits.length > 0 && (
                            <div>
                              <p className="text-[11px] text-slate-400 font-semibold mb-1.5">Credited Personnel:</p>
                              <div className="flex flex-wrap gap-2">
                                {ticket.worker_credits.map((w, idx) => (
                                  <span key={idx} className="text-xs px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/25 flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] text-emerald-400">({w.role})</span>
                                  </span>
                                ))}
                              </div>
                            </div>
                          ...
                          {ticket.proof_media_urls && ticket.proof_media_urls.length > 0 && (
                            <div>
                              <p className="text-[11px] text-slate-400 font-semibold mb-1.5">Proof Media / Inspection Certificates:</p>
                              <div className="flex flex-wrap gap-2">
                                {ticket.proof_media_urls.map((url, i) => {
                                  const isImg = url.endsWith('.jpg') || url.endsWith('.jpeg') || url.endsWith('.png') || url.endsWith('.webp')
                                  return (
                                    <a key={i} href={`http://localhost:8000/${url.replace(/^\//, '')}`} target="_blank" rel="noreferrer" className="text-xs px-2.5 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/25 hover:bg-emerald-500/20 flex items-center gap-1.5 transition-colors">
                                      <span>{isImg ? '🖼️' : '📄'}</span>
                                      <span>Proof Document {i + 1}</span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="p-4 rounded-xl bg-violet-500/5 border border-violet-500/20 h-full">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-xs">
                              <div>
                                ...
                                <p className="text-white font-semibold">{ticket.assigned_institution.name}</p>
                              </div>
                              <div>
                                <p className="text-slate-500 mb-0.5">Type</p>
                                <p className="text-slate-300 capitalize">{ticket.assigned_institution.type}</p>
                              </div>
                              <div>
                                <p className="text-slate-500 mb-0.5">Reputation</p>
                                <p className="text-indigo-400 font-bold">{ticket.assigned_institution.reputation_score.toFixed(3)}</p>
                              </div>
                              <div>
                                <p className="text-slate-500 mb-0.5">Current Load</p>
                                <p className={`font-bold ${ticket.assigned_institution.current_load > 10 ? 'text-red-400' : ticket.assigned_institution.current_load > 5 ? 'text-amber-400' : 'text-emerald-400'}`}>
                            ...
                            {ticket.assigned_institution.domains_of_expertise.length > 0 && (
                              <div className="mt-3 flex flex-wrap gap-1">
                                {ticket.assigned_institution.domains_of_expertise.map(d => (
                                  <span key={d} className={`text-[10px] px-1.5 py-0.5 rounded font-mono border ${d === ticket.domain ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/40' : 'bg-slate-800/80 text-slate-500 border-slate-700'}`}>
                                    {d === ticket.domain ? '★ ' : ''}{d}
                                  </span>
                                ))}
                              </div>
                        ...
                        {/* Activity timeline */}
                        <div className="p-4 rounded-xl bg-black/20 border border-white/5 max-h-64 overflow-y-auto">
                          <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-4 sticky top-0 bg-slate-900/90 py-1 backdrop-blur-sm z-10">Complete Audit Trail ({ticket.events?.length ?? 0})</h4>
                            ...
                            <div className="relative pl-5 space-y-4">
                              ...
                              {ticket.events.map((e, i) => (
                                <div key={i} className="flex items-start gap-3 relative">
                                  <span className={`absolute -left-4 top-1 w-2.5 h-2.5 rounded-full border-2 border-slate-900 ${i === ticket.events.length - 1 ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]' : 'bg-slate-500'}`} />
                                  <div>
                                    <p className="text-xs font-bold text-slate-300 capitalize flex items-center gap-2">
                                      {e.type.replace(/_/g,' ')}
                                    ...
                                    {e.notes && <p className="text-xs text-slate-400 mt-1 leading-relaxed bg-white/5 p-2 rounded border border-white/5">{e.notes}</p>}
                                  </div>
                                </div>
                              ))}
                            </div>
                    ...
                    {/* Action Panel */}
                    <div className="px-5 py-4 bg-black/30 flex gap-2 flex-wrap items-center">
                      {ticket.status !== 'closed' && (
                        <div className="flex items-center gap-2 mr-auto">
                          <button onClick={() => setAssignModalTicket(ticket)} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600/20 text-indigo-300 text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-indigo-600/40 transition-all border border-indigo-500/30">
                      ...
                      {ticket.status !== 'closed' && (
                        <form action={async () => { await closeTicket(ticket.id) }}>
                          <button type="submit" className="inline-flex items-center gap-2 px-5 py-2 bg-slate-700/80 text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-slate-600 transition-all border border-slate-500/40">
  ...
  const RoutingTab = () => (
    <div className="space-y-5">
      <div className="glass-card p-5 flex items-start gap-4 bg-indigo-500/5 border-indigo-500/20 shadow-inner">
        <div className="w-10 h-10 rounded-full bg-indigo-500/20 flex items-center justify-center shrink-0">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg>
        </div>
        <div className="text-sm text-slate-300 leading-relaxed">
          <p className="font-bold text-white mb-1 text-base">Institution Routing Intelligence</p>
      ...
      {workload.length === 0 ? (
        <div className="glass-card p-8 text-center text-slate-500">No institutions registered.</div>
      ) : (
        <div className="grid grid-cols-1 gap-5">
          {workload.map(inst => {
            const totalActive = Object.entries(inst.status_counts).filter(([s]) => s !== 'closed').reduce((a,[,v]) => a+v, 0)
            return (
              <div key={inst.id} className="glass-card p-0 overflow-hidden hover:border-indigo-500/30 transition-colors">
                {/* Header row */}
                <div className="p-5 border-b border-white/5 bg-slate-900/40 flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-extrabold text-white text-lg">{inst.name}</h3>
                      <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider bg-slate-800 border border-slate-700 px-2.5 py-1 rounded-md">{inst.type}</span>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(inst.domains_of_expertise as string[]).map(d => (
                        <span key={d} className="text-xs bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 rounded-md font-mono shadow-sm">{d}</span>
                      ))}
                      {inst.domains_of_expertise.length === 0 && <span className="text-xs text-slate-500 italic">No domain specializations</span>}
                    </div>
                  </div>

                  <div className="flex items-center gap-8 text-right bg-black/20 p-3 rounded-xl border border-white/5">
                    <div>
                      <p className="text-2xl font-black text-indigo-400 leading-none">{inst.reputation_score.toFixed(3)}</p>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-1">Global Rep</p>
                    </div>
                    <div className="w-px h-8 bg-white/10" />
                    <div>
                      <p className={`text-2xl font-black leading-none ${inst.current_load > 10 ? 'text-red-400' : inst.current_load > 5 ? 'text-amber-400' : 'text-emerald-400'}`}>{inst.current_load}</p>
                  ...
                  <div className="lg:col-span-1 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-slate-900/80 border border-white/5 rounded-xl p-3">
                        <p className="text-xl font-black text-white">{inst.total_tickets}</p>
                      ...
                      <div className={`bg-slate-900/80 border rounded-xl p-3 ${inst.sla_breached > 0 ? 'border-red-500/40 bg-red-500/5' : 'border-white/5'}`}>
                      ...
                      <div className={`bg-slate-900/80 border rounded-xl p-3 ${inst.sla_at_risk + inst.sla_critical > 0 ? 'border-amber-500/40 bg-amber-500/5' : 'border-white/5'}`}>
                      ...
                      {Object.entries(inst.status_counts).length === 0 ? (
                        ...
                        <div className="flex flex-col gap-2">
                          {Object.entries(inst.status_counts).sort((a,b)=>b[1]-a[1]).map(([s, c]) => (
                            <div key={s} className="flex items-center justify-between bg-black/20 p-2 rounded-lg">
                          ...
                          {Object.entries(inst.reputation_by_domain).sort((a,b) => b[1]-a[1]).map(([dom, score]) => (
                            <div key={dom}>
                              <div className="flex items-center justify-between text-xs mb-1">
                                <span className="text-slate-300 font-mono truncate">{dom}</span>
                                <span className="font-bold text-indigo-400">{Number(score).toFixed(2)}</span>
                              </div>
                              <div className="h-1.5 bg-slate-900 rounded-full overflow-hidden">
                                <div className="h-full rounded-full bg-indigo-500" style={{ width: `${Number(score) * 100}%` }} />
                              </div>
                            </div>
                          ))}
                        </div>
  ...
  const SlaTab = () => {
    if (!slaRisk || slaRisk.summary.total_active === 0) {
      return (
        <div className="glass-card p-16 text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">SLA Health is Perfect</h2>
          <p className="text-slate-400 max-w-sm mx-auto">There are no active tickets currently being tracked against SLAs, or all tickets have been resolved.</p>
        </div>
      )
    }
    ...
    const sections = [
      ...
      { key: 'safe', label: 'On Track', sub: 'Plenty of time remaining.', items: slaRisk.safe, color: 'border-white/5', icon: '🟢' },
    ]
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Safe', count: slaRisk.summary.safe_count, color: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' },
          ].map(p => (
        ...
        {sections.map(sec => sec.items.length === 0 ? null : (
          <div key={sec.key} className="space-y-3">
            <div className="flex items-end gap-2 px-1">
              ...
              <h3 className="text-sm font-bold text-white tracking-wide">{sec.label} <span className="text-slate-500 ml-1">({sec.items.length})</span></h3>
              <p className="text-xs text-slate-500 ml-2 hidden sm:block">{sec.sub}</p>
            </div>
            <div className={`rounded-2xl border p-2 ${sec.color}`}>
              {sec.items.map(t => (
                <div key={t.id} className="flex flex-wrap items-center gap-4 p-4 rounded-xl bg-slate-950/40 border border-white/5 hover:bg-white/5 transition-colors mb-2 last:mb-0">
                  <div className="w-12 h-12 rounded-full bg-slate-900 flex flex-col items-center justify-center shrink-0 border border-white/10 shadow-inner">
                    <span className="text-[9px] text-slate-500 font-bold">ID</span>
                    <span className="text-sm font-mono text-white">#{t.id}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-bold text-white truncate mb-1">{t.title}</p>
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      {t.domain && <span className="font-mono bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-1.5 py-0.5 rounded shadow-sm">{t.domain}</span>}
                  ...
                  <div className="flex items-center gap-3 shrink-0 bg-black/20 p-2 rounded-xl border border-white/5">
                    <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${statusBadge(t.status)}`}>
                      {t.status.replace(/_/g,' ')}
                    </span>
                    <div className="w-px h-6 bg-white/10" />
                    <button onClick={() => { setAssignModalTicket(tickets.find(tick => tick.id === t.id) ?? null) }} className="text-xs px-3 py-1.5 bg-indigo-600/20 border border-indigo-500/40 text-indigo-300 font-bold rounded-lg hover:bg-indigo-600/40 transition-colors uppercase tracking-wider">
                      Override
                    </button>
                  </div>
                </div>
  ...
  const StaleTab = () => (
    <div className="space-y-5">
      <div className="glass-card p-5 flex items-start gap-4 bg-amber-500/5 border-amber-500/20 shadow-inner">
        ...
        <div className="text-sm text-slate-300 leading-relaxed">
          <p className="font-bold text-white mb-1 text-base">Stale / Stuck Tickets Alert</p>
      ...
      {staleTickets.length === 0 ? (
        <div className="glass-card p-16 text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">No Stale Tickets!</h2>
          <p className="text-slate-400 max-w-sm mx-auto">All actively routed and accepted tickets have received recent updates.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {staleTickets.map(t => (
            <div key={t.id} className="glass-card p-0 border-amber-500/30 overflow-hidden flex flex-col group">
              <div className="p-5 flex-1 border-b border-white/5 group-hover:bg-white/5 transition-colors">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-mono text-slate-500 bg-slate-900 px-2 py-0.5 rounded border border-white/5">#{t.id}</span>
                ...
                <h3 className="font-bold text-white text-[15px] mb-2 leading-snug">{t.title}</h3>

                <div className="space-y-2 mt-4 p-3 bg-black/20 rounded-xl border border-white/5">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-slate-500">Institution:</span>
                    <span className="text-slate-200 font-semibold text-right">{t.assigned_institution}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                  ...
                  {t.sla_hours_remaining != null && (
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className={`font-mono font-semibold ${t.sla_hours_remaining < 0 ? 'text-red-400' : 'text-slate-300'}`}>
              ...
              <div className="p-3 bg-slate-950 flex items-center justify-between gap-2">
                <button onClick={() => { setAssignModalTicket(tickets.find(tick => tick.id === t.id) ?? null) }} className="flex-1 text-xs px-3 py-2 bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 font-bold rounded-lg hover:bg-indigo-600/40 transition-colors uppercase tracking-wider">
                  Re-Assign
                </button>
                <form className="flex-1 flex" action={async () => { await closeTicket(t.id) }}>
                  <button type="submit" className="w-full text-xs px-3 py-2 bg-slate-800 border border-slate-700 text-slate-300 font-bold rounded-lg hover:bg-slate-700 transition-colors uppercase tracking-wider">
                    Close
                  </button>
                </form>
              </div>
  ...
  return (
    <div className="space-y-6 pb-10 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="page-title">Government Control Panel</h1>
          <p className="page-subtitle mt-1">Civic issue management & routing intelligence for Jharkhand</p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/dashboard/government/analytics"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 text-sm font-semibold hover:bg-indigo-600/30 transition-all shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
            Deep Analytics
          </Link>
          <button onClick={() => window.location.reload()}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800/80 border border-white/10 text-slate-300 text-sm font-semibold hover:bg-slate-700 transition-all shadow-sm">
