...
function statusBadge(status: string) {
  ...
  return styles[status] ?? 'bg-slate-500/20 text-slate-500 border border-slate-500/30'
}
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="page-subtitle">Tickets assigned to your institution â€” accept, work on, upload proof, and credit your team.</p>
      </div>
          ...
          <div key={ticket.id} className="glass-card p-5">
            <div className="flex justify-between items-start gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-base text-slate-900 truncate">{ticket.title}</h3>
                ...
                <div className="mt-2 flex items-center gap-3 flex-wrap">
                  {ticket.domain && (
                    <span className="text-xs font-mono bg-orange-50 text-orange-700 border border-orange-500/20 px-2 py-0.5 rounded">
                      {ticket.domain}
                    </span>
                  )}
                  {ticket.severity_score != null && (
                    <span className="text-xs text-slate-500">
                      Severity: <span className="text-slate-600">{Number(ticket.severity_score).toFixed(2)}</span>
                    </span>
                  )}
                </div>
              </div>
            ...
            {ticket.proof_media_urls && ticket.proof_media_urls.length > 0 && (
              <div className="mt-3 p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                <p className="text-xs font-bold text-emerald-400 mb-1">âœ… Proof Uploaded ({ticket.proof_media_urls.length} file{ticket.proof_media_urls.length !== 1 ? 's' : ''})</p>
                {ticket.completion_notes && <p className="text-xs text-slate-500 mt-1">{ticket.completion_notes}</p>}
              </div>
            )}
            ...
            <div className="flex flex-col gap-3 mt-4">
              <div className="flex gap-2 flex-wrap">
                ...
                {ticket.status === 'routed' && (
                  <form action={async () => {
                    ...
                    <button className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-600/80 text-slate-900 text-sm font-semibold rounded-xl hover:bg-emerald-600 transition-all border border-emerald-500/40 shadow-sm shadow-emerald-500/20">
                ...
                {ticket.status === 'accepted' && (
                  <form action={async () => {
                    ...
                    <button className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600/80 text-slate-900 text-sm font-semibold rounded-xl hover:bg-cyan-600 transition-all border border-cyan-500/40 shadow-sm shadow-cyan-500/20">
              ...
              {/* Upload Proof Form â€” shown when in_progress or accepted */}
              {(ticket.status === 'in_progress' || ticket.status === 'accepted') && (
                <form action={async (formData: FormData) => {
                  ...
                  <h4 className="text-sm font-bold text-teal-300">ðŸ“‹ Submit Completion Report</h4>
                  <textarea
                    name="completion_notes"
                    placeholder="Describe what was done to resolve this issue..."
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-700 placeholder-slate-600 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500/50 resize-none h-24"
                  />
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-semibold text-slate-500 block mb-1">Worker Names (comma-separated)</label>
                      <input
                        name="worker_names"
                        placeholder="e.g. Rahul Kumar, Priya Singh"
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-600 placeholder-slate-600 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500/50"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-slate-500 block mb-1">Roles (comma-separated)</label>
                      <input
                        name="worker_roles"
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 block mb-1">Proof Photo / Document (optional)</label>
                    <input
                      type="file"
                      name="proof"
                      accept="image/*,application/pdf"
                      className="w-full text-sm text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-teal-500/10 file:text-teal-300 file:font-semibold hover:file:bg-teal-500/20 cursor-pointer"
                    />
                  </div>
                  <button type="submit" className="inline-flex items-center gap-1.5 px-4 py-2 bg-teal-600/80 text-slate-900 text-sm font-semibold rounded-xl hover:bg-teal-600 transition-all border border-teal-500/40">
              ...
              {ticket.status === 'piloting' && (
                <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20 text-sm text-amber-300">
                  â³ Completion report submitted â€” awaiting government verification.
                </div>
              )}

              {ticket.status === 'verified' && (
                <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-sm text-emerald-300">
                  âœ… Work verified by government. Ticket will be closed shortly.
                </div>
              )}
            </div>
                ...
                <ul className="text-xs text-slate-500 flex flex-col gap-1.5">
                  {ticket.events.map((event: TicketEvent, i: number) => (
                    <li key={i} className="flex items-start gap-1.5">
                      ...
                      <span>
                        <span className="font-semibold text-slate-500">{String(event.type).replace(/_/g, ' ')}</span>
                        {' â€” '}{new Date(event.time).toLocaleString()}
